...
func (sb *StatementBase) Name() string {
	return NodeNameUnknow
}
...
const (
	NodeNameSelect = "SELECT"
)

func (node *Select) Name() string {
	return NodeNameSelect
}
...
const (
	NodeNameDatabaseCreate = "DATABASE_CREATE"
	NodeNameDatabaseDrop   = "DATABASE_DROP"
)

func (node *DBDDL) Name() string {
	switch node.Action {
	case CreateStr:
		return NodeNameDatabaseCreate
	case DropStr:
		return NodeNameDatabaseDrop
	}
	return NodeNameUnknow
}
...
const (
	NodeNameTableCreate = "TABLE_CREATE"
	NodeNameTableDrop   = "TABLE_DROP"
)

func (node *DDL) Name() string {
	switch node.Action {
	case CreateStr:
		return NodeNameTableCreate
	case DropStr:
		return NodeNameTableDrop
	}
	return NodeNameUnknow
}
...
// Format formats the node.
func (t *DatabaseOption) Format(buf *TrackedBuffer) {
	if t.Engine != "" {
		buf.Myprintf("ENGINE = %s", t.Engine)
	}
}

func (t *DatabaseOption) walkSubtree(visit Visit) error {
	return nil
}
...
// Format formats the node.
func (t *TableOption) Format(buf *TrackedBuffer) {
	if t.Engine != "" {
		buf.Myprintf("ENGINE = %s", t.Engine)
	}
	if t.Formatter != "" {
		buf.Myprintf("(%s)", t.Formatter)
	}
}

func (t *TableOption) walkSubtree(visit Visit) error {
	return nil
}
...
const (
	NodeNameShowTables    = "SHOWTABLES"
	NodeNameShowDatabases = "SHOWDATABASES"
)

func (node *Show) Name() string {
	switch strings.ToLower(node.Type) {
	case "databases":
		return NodeNameShowDatabases
	case "tables":
		return NodeNameShowTables
	}
	return node.Type
}
